        ...
        IEventSyncer eventSyncer) : IInstanceSyncer
    {
        public async Task ProcessSyncJob(SyncJobCompositeModel syncJobCompositeModel, CancellationToken stoppingToken)
        {
            try
            {
                ...
                await storeHubDataAdapter.UpdateAsync(syncJobCompositeModel.SyncModel);
        ...
        private async Task<bool> SyncAllRecords(SyncJobCompositeModel syncJobCompositeModel, CancellationToken stoppingToken)
        {
            try
            {
                ...
                foreach (var item in octopusSpaces)
                {
                    ...
                    var spaceModel = await storeHubDataAdapter.GetByOctopusIdAsync<SpaceModel>(item.OctopusId);
                    ...
                    var spaceToTrack = item.Id > 0 ? await storeHubDataAdapter.UpdateAsync(item) : await storeHubDataAdapter.InsertAsync(item);
        ...
        private async Task LogException(string message, SyncJobCompositeModel syncJobCompositeModel)
        {
            ...
            await storeHubDataAdapter.InsertAsync(syncLogModelFactory.MakeErrorLog(formattedMessage, syncJobCompositeModel.SyncModel.Id));
        }
    }
}
